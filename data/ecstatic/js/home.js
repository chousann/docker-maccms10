...
var MAC={
    ...
    'Shorten': {
        ...
        'Get':function(url,call){
            ...
            MAC.Ajax('//api.maccms.com/shorten/?callback=callback&url='+ encodeURIComponent(url),'get','jsonp','',function(r){
                if (r.code == 1) {
                    if($('.mac_shorten').length>0) {
                        $('.mac_shorten').append(r.data.url_short);
                    }
                    if(call){
                        call(r);
                    }
    ...
    'History': {
        ...
        'Init':function(){
            ...
            html +='<ul class="ec-plays ecbgk clearfix ec-lf13">'
            if(jsondata.length > 0){
                for($i=0; $i<jsondata.length; $i++){
                    html +='<li class="ec-top1 clearfix">';
                    html +='<a href="'+jsondata[$i].link+'" title="'+jsondata[$i].name+'"><div class="ec-plays-pic ec-jabj" style="background-image:url('+jsondata[$i].pic+')"></div><div class="ec-plays-titbox"><p class="ec-plays-title">'+jsondata[$i].name+'</p></div></a></li>';
                }
            }else{
			    html +='<div class="ec-kong"><div></div><p><a href="javascript:MAC.GoBack()" class="nexback">空空滴，先去逛一波吧~</a></p></div>';
            }
            $('.mac_history').prepend(html);
